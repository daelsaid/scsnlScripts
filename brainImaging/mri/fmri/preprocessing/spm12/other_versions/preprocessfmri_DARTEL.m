...
% ruiyuan, 2019-02-09, add DATEL module 
...
pipelinefamily = {'swar', 'swavr', 'swgcar', 'swgcavr', 'swdgcar'...
...
%               fclose(fid);
      ...
      %%%%%%---------------------------------------------------        
        ...
        case 'w'
          ...
          preprocessfmri_normalizeD(wholepipeline, currentdir, template_path, boundingboxdim, [pipeline, inputimgprefix], inputimg_file, meanimg_file, temp_dir, SPGRfile_file, spm_path,project_dir);
...
%             end
       ...
       case 'd'
           ...
           %%% --- copy arI.nii to tmp file as garI.nii
              listfile_file = dir(fullfile(temp_dir, [prevprefix, 'I.nii']));
              unix(sprintf('cp -af %s %s', fullfile(temp_dir, listfile_file(1).name), fullfile(temp_dir, ['d', listfile_file(1).name])));


             listfile_file = dir(fullfile(SPGRdir, ['seg' ,'_', spm_version],[ 'u_rc1',SPGRfilename,'_Template.nii']));
          if isempty(listfile_file)
            error('Error: no segmentation has been done, use preprocessmri.m');
          else
            disp('------------------------------------------------------------------');
