function scsnl_groupoutlier_rest(ConfigFile)
...
%                                      ['visit',visit],['session',session], 'glm', 'stats_spm12', stats_folder);
       ...
       stats_dir{statscnt, subcnt} = fullfile(project_folder, 'results','restfmri','participants',subject,...
                                      ['visit',visit],['session',session],'seedfc','rest', 'stats_spm8', stats_folder,'stats');
...
%- output path 
%------------------------------------------------------------------------
output_dir = fullfile(project_folder, 'results','restfmri', 'groupstats', output_folder_name);
...
tmp_v_mask = spm_vol(fullfile(stats_dir{1,1},'mask.nii'));
  ...
  warning('There is no mask.nii file. Program is going to find mask.img ');
  tmp_v_mask = spm_vol(fullfile(stats_dir{1,1},'mask.img'));
end
...
for subcnt = 1: numsubj
    ...
    tmp_v_mask = spm_vol(fullfile(stats_dir{1,subcnt},'mask.nii'));
     ...
     warning('There is no mask.nii file. Program is going to find mask.img ');
     tmp_v_mask = spm_vol(fullfile(stats_dir{1,subcnt},'mask.img'));
...
fin_mask_index = find(all_mask == numsubj);
...
for concnt = 1:numcontrast
    ...
    for subcnt = 1:numsubj
     try
      ConImage{count} = spm_select('FPList', stats_dir{statscnt, subcnt}, ...
        ...
        warning('There is no con*.nii file. Program is going to find con*.img ');
  ...
  scsnl_art_groupoutlier_rest(ConImage, MaskImage ,Groupscale, output_dir,  contrasts.contrastNames{concnt});
end
