...
% Rui Yuan, 2018-02-10,
% -------------------------------------------------------------------------
%Edited on 2019-03-13  by Carlo de los Angeles to accomodate more columns in the subject list to denote which functional is which run, and a contrast file
%
% i. e. PID,Visit,Session,Run1,Run2,Contrast_file
%	subject_id,1,1,addition_block1,addition_block_2_redo,contrastfile_2runs.mat
...
% Get the subjects, sesses in cell array format
        ...
        %subjectlist       = dlmread(subjectlist,',',1);
        opt = {'Delimiter',','};
        ...
        fclose(fid);
        idx=size(subjectlist,2);
        contrastmat         = subjectlist{idx}{subject_i}
        subject           = subjectlist{1}{subject_i};
        subject           = char(string(subject));
        subject           = char(pad(string(subject),4,'left','0'));%<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        visit             = subjectlist{2}{subject_i};
        session           = subjectlist{2}{subject_i};

        numsub           = 1;
        if size(subjectlist,2) > 3
            for i=4:size(subjectlist,2)-1
                runs{i-3} = subjectlist{i}{subject_i}
            end 
	else
            runs = ReadList(strtrim(paralist.runlist))
        end
