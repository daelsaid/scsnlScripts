...
% YZ: this is for PNC dataset only 
...
for isubj = 1:numsub
  ...
  for itask = 1:num_task+4
      if itask <= num_task
          if strcmp(task_names{itask},'incorrect')
              continue;
          end
      end
    ...
    for isess = 1:num_sess
      % Yuan add
      if itask == num_task+1 % combine fear and angry
          iG_exp = ['^Sn\(', num2str(isess), ').', '(fear|angry)'];
      elseif itask == num_task+2 % combine negative emotion conditions
          iG_exp = ['^Sn\(', num2str(isess), ').', '(fear|angry|sad)'];  
      elseif itask == num_task+3 % combine four emotion-salient conditions
          iG_exp = ['^Sn\(', num2str(isess), ').', '(fear|angry|sad|happy)'];
      elseif itask == num_task+4 % combine all conditions 
          iG_exp = ['^Sn\(', num2str(isess), ').', '(neutral|fear|angry|sad|happy)'];          
      else
...
%       iG_exp = ['^Sn\(', num2str(isess), ').', task_names{itask}];
  ...
  %Yuan add
  task_names{num_task+1} = 'FA';
  task_names{num_task+2} = 'FAS';
  task_names{num_task+3} = 'FASH';
  task_names{num_task+4} = 'NFASH'; 
  
  save(fullfile(result_dir,[result_fname,'_',subject,'.mat']),'C', 'P', 'roi_names','roi_beta_series', 'task_names');
end
