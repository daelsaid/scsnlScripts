...
% -------------------------------------------------------------------------
% YZ modified this version to use a parcellation rather than a list of rois to do network construction

function fconnect_roi_roi_mask(SubjectI, ConfigFile)
...
extract_opt = strtrim(paralist.extract_opt);
...
roi_d = spm_read_vols(spm_vol(roi_list));
% numroi = max(roi_d(:));
numroi = length(unique(roi_d(:)))-1;
...
for irun = 1:numrun
    ...
    all_roi_ts = extract_ROI_timeseries_YZ(roi_list, temp_dir, 1, ...
        0, newpipeline, data_type, extract_opt);
    all_roi_ts = all_roi_ts';
    all_roi_ts = all_roi_ts(numtrunc(1)+1:end-numtrunc(2), :); % truncate
    NumVolsKept = size(all_roi_ts, 1);
%     all_roi_ts = all_roi_ts - repmat(mean(all_roi_ts, 1), NumVolsKept, 1); % demean
    ...
    global_ts = global_ts - repmat(mean(global_ts, 1), NumVolsKept, 1); 
    ...
    %-covariate matrix
    cov_mtx = [global_ts, wm_csf_ts, mvmnt, ones(NumVolsKept,1)];
...
c     = fix(clock);
disp('==================================================================');
fprintf('resting-state roi-roi network construction finishes at %d/%02d/%02d %02d:%02d:%02d\n',c);
disp('==================================================================');
clear all;
close all;
