...
% YZ: this if for HCP dataset only
% -------------------------------------------------------------------------
...
for irun = 1:numrun
    ...
    if bandpass_include == 1
        ...
        bandpass_final_SPM_HCP(TR_value, fl_value, fh_value, temp_dir, pipeline, data_type);
    ...
    [all_roi_ts, roi_name] = extract_ROI_timeseries_hcp(roi_file, temp_dir, 1, ...
         0, newpipeline, data_type);
    ...
    [wm_csf_ts, wm_csf_roi_name] = extract_ROI_timeseries_hcp_median(wm_csf_roi_file, temp_dir, 1, ...
            0, newpipeline, data_type);
    wm_csf_ts = wm_csf_ts';
    
     %-Truncate ROI and white matter and CSF signals
    ...
    %-wm csf siganl demean
    wm_csf_ts = wm_csf_ts - repmat(mean(wm_csf_ts, 1), NumVolsKept, 1);
    
    % Note that roi timeseries were not demeaned for hcp data (keep consistent with tianwen script)
    %=====================
...
%     NumVolsKept = length(global_ts);
    ...
    cov_mtx = [wm_csf_ts mvmnt ones(NumVolsKept, 1)];
