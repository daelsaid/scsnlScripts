function bandpass_final_SPM_HCP(TR,fl,fh,dataloc,input_prefix,image_type)
...
if ~isempty(files) 
  ...
  if flength == 1
    nifti4Dto3D_HCP(pwd, input_prefix); 
    files = dir([input_prefix '*.' image_type]);
    flength = length(files);
  end
  P = cell(flength,1);
...
%-get data temporal mean
data_mean = mean(data, 4); % Yuan add
...
for k = 1:flength
  ...
  %%% add back the temporal mean
  spm_write_vol(V(k),(squeeze(data_tmp(:,:,:,k)) + data_mean)); % Yuan edit
end
