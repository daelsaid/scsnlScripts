function SPM=PPPI_wc(parameterstructure,structfile)

% modified to fix matlab2017 incompatiblity issue
% changed PPPI_checkstruct to PPPI_checkstruct_wc
...
%
% See PPPI_checkstruct_wc.m for more options.
%
% FOR DEFAULTS, see PPPI_checkstruct_wc.m code.
%
%EXAMPLE:
%
...
if exist('parameterstructure','var') && (~isstruct(parameterstructure) && exist(parameterstructure,'file'))
    ...
    if ~isfield(parameterstructure,'correct') || parameterstructure.correct~=1
        try
            [P, errorval]=PPPI_checkstruct_wc(parameterstructure,structfile);
        catch
            try
                [P, errorval]=PPPI_checkstruct_wc(parameterstructure);
            catch
...
for z=1:Sessions
    ...
    %======================================================================
    if strcmp(P.analysis,'psy')
        dlmwrite_wc([P.subject '_' region1 '_session' num2str(z) '_' P.method '_PPI_regressors.txt'],[OUT.PPI.C OUT.P.C OUT.Y.C OUT.C.C],' ');
        save([P.subject '_' region1 '_session' num2str(z) '_' P.method '_PPI_regressors.mat'],'OUT','-v7.3');
    elseif strcmp(P.analysis,'phy')
        dlmwrite_wc([P.subject '_' region1 '_and_' region2 '_session' num2str(z) '_' P.method '_PPI_regressors.txt'],[OUT.PPI.C OUT.P.C OUT.Y.C OUT.C.C],' ');
        save([P.subject '_' region1 '_and_' region2 '_session' num2str(z) '_' P.method '_PPI_regressors.mat'],'OUT','-v7.3');
    else
        dlmwrite_wc([P.subject '_' region1 '_and_' region2 '_session' num2str(z) '_' P.method '_PPPI_regressors.txt'],[OUT.PPI.C OUT.P.C OUT.Y.C OUT.C.C],' ')
