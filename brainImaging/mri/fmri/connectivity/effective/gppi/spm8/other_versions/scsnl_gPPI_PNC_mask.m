function scsnl_gPPI_yuan(SubjectI,Config_File)
...
%-Yuan, 10/08/2018: (1) do not delete existing files in subject_gppi_stats
%folder, otherwise, completed ROIs will be deleted; (2) use an external
%mask
...
% %         unix('rm -rf *.*');
        ...
        if ~isempty(maskfile)
            if exist(maskfile,'file') == 0
                 error('>>>>>>>> cannot find the mask file');
            end
            P.FLmask =1;
            P.maskdir=maskfile;
            save(['gPPI_', subject, '_analysis_', roi_name{i_roi}, '.mat'], 'P');
            PPPI_yuan(['gPPI_', subject, '_analysis_', roi_name{i_roi}, '.mat']);
        else
            save(['gPPI_', subject, '_analysis_', roi_name{i_roi}, '.mat'], 'P');
            PPPI_yuan(['gPPI_', subject, '_analysis_', roi_name{i_roi}, '.mat']);
        end
