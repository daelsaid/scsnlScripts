...
# Edited  by: Lang     2019-03-18 (for SPM12 (swar pipeline) and reading subject ID) 
# -------------------------------------------------------------------------

# Project dir
projectdir = '/oak/stanford/groups/menon/projects/lchen32/2019_TD_MD_AddSub_Similarity/'

# Path to subjectlist file
subjectlist = '/oak/stanford/groups/menon/projects/lchen32/2019_TD_MD_AddSub_Similarity/data/subjectlist/Subjectlist.csv'

# List of sessions you are running for. Do not need to be present for all subjects
sessions = ['addition_block','subtraction_block']
...
for s in range(len(subjects)):
    for t in range(len(tasks)):
        ...
        strsubj = str(subject).rjust(4,'0')
        strvisit = 'visit' + visit
        strsession = 'session' + session
        run_dir = op.join(projectdir, 'data/imaging/participants', strsubj, strvisit, strsession, 'fmri', tasks[t])
        #for SPM12 the rp_I file is under the task folder/pipeline_spm12
        motdata = np.loadtxt(op.join(run_dir, 'swar_spm12','rp_I.txt'))
        raw_dir = op.join('/oak/stanford/groups/menon/rawdata/scsnl', strsubj, strvisit, strsession, 'fmri', tasks[t])
