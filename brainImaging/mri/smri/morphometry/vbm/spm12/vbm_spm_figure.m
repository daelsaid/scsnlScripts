function varargout= vbm_spm_figure(varargin)
...
%=======================================================================
% F = vbm_spm_figure('Create',Tag,Name,Visible)
%-Condition arguments
...
F = vbm_spm_figure('CreateWin',Tag,Name,Visible);
vbm_spm_figure('CreateBar',F);
vbm_spm_figure('FigContextMenu',F);
varargout = {F};


case 'findwin'
%=======================================================================
% F=vbm_spm_figure('FindWin',F)
% F=vbm_spm_figure('FindWin',Tag)
...
if isempty(F)
    if ischar(Tag)
        ...
        case 'Graphics'
            F = vbm_spm_figure('Create','Graphics','Graphics');
        case 'DEM'
            F = vbm_spm_figure('Create','DEM','Dynamic Expectation Maximisation');
        case 'DFP'
            F = vbm_spm_figure('Create','DFP','Variational filtering');
        case 'FMIN'
            F = vbm_spm_figure('Create','FMIN','Function minimisation');
        case 'MFM'
            F = vbm_spm_figure('Create','MFM','Mean-field and neural mass models');
        case 'MVB'
            F = vbm_spm_figure('Create','MVB','Multivariate Bayes');
        case 'SI'
            F = vbm_spm_figure('Create','SI','System Identification');
        case 'Interactive'
            F = spm('CreateIntWin');
        end
    end
...
if ~isempty(F), F = vbm_spm_figure('FindWin',F); end
...
vbm_spm_figure('Print',F,psname);

case 'newpage'
%=======================================================================
% [hNextPage, hPrevPage, hPageNo] = vbm_spm_figure('NewPage',h)
if nargin<2 || isempty(varargin{2}) error('No handles to paginate')
else h=varargin{2}(:)'; end

%-Work out which figure we're in
F = vbm_spm_figure('ParentFig',h(1));
...
if isempty(hNextPage)
    ...
    hNextPage = uicontrol(F,'Style','Pushbutton',...
        ...
        'Callback','vbm_spm_figure(''TurnPage'',''+1'',gcbf)',...
...
case 'turnpage'
%=======================================================================
% vbm_spm_figure('TurnPage',move,F)
...
%=======================================================================
% vbm_spm_figure('DeletePageControls',F)
if nargin<2, F='Graphics'; else F=varargin{2}; end
F = vbm_spm_figure('FindWin',F);
if isempty(F), error('No Graphics window'), end
...
case 'currentpage'
%=======================================================================
% n = vbm_spm_figure('CurrentPage', F)
if nargin<2, F='Graphics'; else F=varargin{2}; end
F = vbm_spm_figure('FindWin',F);
if isempty(F), error('No Graphics window'), end
...
case 'watermark'
%=======================================================================
% vbm_spm_figure('WaterMark',F,str,Tag,Angle,Perm)
...
case 'getwinscale'
%=======================================================================
% WS = vbm_spm_figure('GetWinScale')
warning('vbm_spm_figure(''GetWinScale''... is Grandfathered: use spm(''WinScale''')
varargout = {spm('WinScale')};


case 'fontsizes'
%=======================================================================
% FS = vbm_spm_figure('FontSizes',FS)
warning('vbm_spm_figure(''FontSizes''... is Grandfathered: use spm(''FontSizes''')
...
uimenu(t1,'Label','Gray',     'CallBack','vbm_spm_figure(''ColorMap'',''gray'')');
uimenu(t1,'Label','Hot',      'CallBack','vbm_spm_figure(''ColorMap'',''hot'')');
uimenu(t1,'Label','Pink',     'CallBack','vbm_spm_figure(''ColorMap'',''pink'')');
uimenu(t1,'Label','Jet','CallBack','vbm_spm_figure(''ColorMap'',''jet'')');
uimenu(t1,'Label','Gray-Hot', 'CallBack','vbm_spm_figure(''ColorMap'',''gray-hot'')');
uimenu(t1,'Label','Gray-Cool','CallBack','vbm_spm_figure(''ColorMap'',''gray-cool'')');
uimenu(t1,'Label','Gray-Pink','CallBack','vbm_spm_figure(''ColorMap'',''gray-pink'')');
uimenu(t1,'Label','Gray-Jet', 'CallBack','vbm_spm_figure(''ColorMap'',''gray-jet'')');
t1=uimenu(t0,'Label','Effects');
uimenu(t1,'Label','Invert','CallBack','vbm_spm_figure(''ColorMap'',''invert'')');
uimenu(t1,'Label','Brighten','CallBack','vbm_spm_figure(''ColorMap'',''brighten'')');
uimenu(t1,'Label','Darken','CallBack','vbm_spm_figure(''ColorMap'',''darken'')');
% uimenu( F,'Label','Clear','HandleVisibility','off','CallBack','vbm_spm_figure(''Clear'',gcbf)');
t0=uimenu( F,'Label','SPM-Print','HandleVisibility','off');
%uimenu( F,'Label','SPM-Print','HandleVisibility','off','CallBack','vbm_spm_figure(''Print'',gcbf)');
t1=uimenu(t0,'Label','default print file','HandleVisibility','off','CallBack','vbm_spm_figure(''Print'',gcbf)');
t1=uimenu(t0,'Label','other print file','HandleVisibility','off','CallBack','vbm_spm_figure(''PrintTo'',vbm_spm_figure(''FindWin'',''Graphics''))');
...
case 'figcontextmenu'
%=======================================================================
% h = vbm_spm_figure('FigContextMenu',F)
if nargin<2
    F = get(0,'CurrentFigure');
    if isempty(F), error('no figure'), end
else
    F = vbm_spm_figure('FindWin',varargin{2});
...
case 'colormap'
%=======================================================================
% vbm_spm_figure('ColorMap',ColAction,h)
...
case 'graphicshandle'
%=======================================================================
% h = vbm_spm_figure('GraphicsHandle',F)
if nargin<2, F=gcbf; else F=vbm_spm_figure('FindWin',varargin{2}); end
if isempty(F), return, end
