...
if [ -z "$strspmver" ]; then
    echo 'Variable paralist.spmversion not found in configuration file' $2
    echo 'Assuming ' $1 ' is a non-SPM matlab script' 
    strspmver="'_'"
fi
IFS="'"
tokens=($strspmver)
...
if [ ! -f $jobdir ]; then
    echo 'WARNING: Job directory doesn't exist! Creating one!'
    mkdir -p $jobdir
fi
#################
