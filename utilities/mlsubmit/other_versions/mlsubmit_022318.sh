...
#################
#check if user the configuration file exist
if [ ! -f $2 ]; then
...
then
	...
	do
    		...
    		sbatch -J $JOBNAME -o $JOBOUTPUT -e $JOBERROR -t $JOBTIME -N $JOBNNODES --qos=$JOBQOS --mem-per-cpu=$JOBMEM -p $JOBPARTITION --wrap="matlab -nosplash -noFigureWindows -nodisplay -r $'addpath(genpath(\'$scriptfullfile\')); which $scriptname; $scriptname($i,\'$configfullfile\'); exit;'"    		
                sleep 30
...
then
	...
	sbatch -J $JOBNAME -o $JOBOUTPUT -e $JOBERROR -t $JOBTIME -N $JOBNNODES --qos=$JOBQOS --mem-per-cpu=$JOBMEM -p $JOBPARTITION --wrap="matlab -nosplash -noFigureWindows -nodisplay -r $'addpath(genpath(\'$scriptfullfile\')); which $scriptname; $scriptname(\'$configfullfile\'); exit;'"
fi
echo '********************************************************'
#################
