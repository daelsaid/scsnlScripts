...
then
	...
	do
    		...
    		sbatch -J $JOBNAME -o $JOBOUTPUT -e $JOBERROR -t $JOBTIME -N $JOBNNODES --qos=$JOBQOS --mem-per-cpu=$JOBMEM -p $JOBPARTITION --wrap="matlab -nosplash -noFigureWindows -nodisplay -r $'addpath(genpath(\'$scriptdir\')); which $1; $scriptname($i,\'$configfullfile\'); exit;'"    		
                sleep 15
