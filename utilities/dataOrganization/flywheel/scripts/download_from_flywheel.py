...
#Function for modifying a filewheel acquistion's classification and measurement based on file names
...
for exam in exams_thin:
	...
	for acquisition in study_template:
		for fw_acq in fw_acqs:
			...
			if fw_acq['label'] == acquisition or fw_acq['label'] == acquisition+'_func':
			...
			if acquisition in fw_acq['label'] and 'redo' in fw_acq['label']:
				for download_pair in study_template[acquisition]:
					for file in fw_acq['files']:
						...
						if download_pair[0] in file['name']:
							modify_file_class_by_template(fw_acq['_id'],file['name'],download_pair[1])
							os.makedirs(split_path(pjoin(scanidpath,download_pair[1].replace('*',file['name'])).replace(acquisition,fw_acq['label'].replace('_func','').replace(' ','_')))[0],exist_ok=True)
							pathname = checkpath(pjoin(scanidpath,download_pair[1].replace('*',file['name'])).replace(acquisition,fw_acq['label'].replace('_func','').replace(' ','_')))
							print('downloading %s'%pathname)
							fw.download_file_from_acquisition(fw_acq['_id'], file['name'], pathname)
...
for scanid_list in downloaded_scanids:
	...
	print('processing %s downloaded data'%scanid)
	if scanid in already_processed:
