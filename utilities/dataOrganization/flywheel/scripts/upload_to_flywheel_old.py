'''This script is for backfilling flywheel with data from OAK, it will upload all acquisitions that are present in rawdata scsnl, but not present on fw'''
...
study_scanids = {}
study_lists_file_names = os.listdir('/oak/stanford/groups/menon/scsnlscripts/utilities/dataOrganization/study_sublists')
for file_name in study_lists_file_names:
	study_name = file_name.split('_')[0]
	file = open(pjoin('/oak/stanford/groups/menon/scsnlscripts/utilities/dataOrganization/study_sublists',file_name),'r')
	scans= file.readlines()
	scans = [x.strip() for x in scans]
	study_scanids[study_name] = scans	
			...
			if folder in behav_targets:
				...
				for behav_file in behav_files:
					if not os.path.isfile(pjoin(behav_path,behav_file)):
						...
						continue
					print('uploading %s to acq %s'%(pjoin(behave_path,behav_file),upload_acq))
...
#iterate through rawdata scanids and identity and upload those that aren't present
for scanid in rawdata_scanids:
	print('uploading %s'%scanid)
	if scanid in fw_scanids:
		print('already uploaded')
		continue
	#check if scanid associated with study
	study = 'unsorted'
	for current_study in study_scanids:
		if scanid in study_scanids[current_study]:
			study = current_study
			break

	study_id = project_thin[study]
