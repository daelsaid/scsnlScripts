'''This script is for backfilling flywheel with data from OAK,
 it will upload all data in rawdata to the unsorted project on flywheel if that subject is not already on flywheel'''
...
print('%s scanids found in rawdata'%str(len(rawdata_scanids)))
...
def upload_fmri(session_id,scanid):
	...
	for path in fmri_image_paths:
		...
		for i in range(len(path_split)):
			if 'unnormalized' in path_split[i] and path_split[i] != 'unnormalized_cor':
				run = path_split[i-1]
